...
with tab2:
    ...
    def generate_subtitle_clips(subtitle_path, video_width, video_height, font_path):
        ...
        # 🔥 Smart Ratio Check & Adjustments
        if video_height > video_width: 
            # 9:16 (ဒေါင်လိုက်)
            wrap_w, pos_y, f_div = 35, 0.70, 18
        else: 
            # 16:9 (အလျားလိုက်)
            wrap_w, pos_y, f_div = 55, 0.75, 22 # စာလုံးရေဖြတ်တောက်မှုကို ၅၅ လုံးသို့ လျှော့ချပြီး နေရာကို အပေါ်တင်လိုက်သည်
        ...
        for line in subs:
            ...
            # 🔥 စာသားကို အလိုအလျောက် အကြောင်းခွဲခြင်း
            wrapped_text = textwrap.fill(line.text.replace("\\N", " "), width=wrap_w)
            
            # စာသားပတ်ဝန်းကျင် ပုံရိပ်တည်ဆောက်မှု
            text_w, text_h = int(video_width * 0.95), int(video_height * 0.45)
...
with tab3:
    ...
    if st.session_state.user_info is None: show_login_ui("t3_l")
    else:
...
with tab4:
    ...
    if st.session_state.user_info is None: show_login_ui("t4_l")
    else:
        st.success(f"✅ VIP အကောင့်: {st.session_state.user_info}")
        if st.button("Logout", key="out_b"):
            st.session_state.user_info = None
            st.rerun()
        
        col_v, col_a = st.columns(2)
        with col_v: v_in = st.file_uploader("Video ရွေးပါ", type=["mp4", "mov"], key="vm_u")
        with col_a: a_in = st.file_uploader("Audio ရွေးပါ", type=["mp3", "wav", "m4a"], key="am_u")
        
        speed = st.select_slider("အသံ အနှေး/အမြန်:", options=["0.5x", "0.75x", "1.0x", "1.25x", "1.5x", "2.0x"], value="1.0x")
        keep_bg = st.checkbox("မူရင်း Background အသံထားမည်", value=True, key="bg_c")

        if v_in and a_in and st.button("Merge Now", key="m_b"):
            with st.spinner("Processing..."):
                ...
                try:
                    ...
                    if speed != "1.0x":
                        ...
                        subprocess.run(["ffmpeg", "-y", "-i", t_a, "-filter:a", f"atempo={rate}", "-vn", "t_p.mp3"])
                        final_a = "t_p.mp3"
